...
[![Latest Release](https://img.shields.io/github/v/release/SanghunWoo-23/SPPS-PLANNER?style=for-the-badge&label=release)](https://github.com/SanghunWoo-23/SPPS-PLANNER/releases/latest)
[![CI](https://img.shields.io/github/actions/workflow/status/SanghunWoo-23/SPPS-PLANNER/ci.yml?style=for-the-badge&label=CI)](https://github.com/SanghunWoo-23/SPPS-PLANNER/actions/workflows/ci.yml)
...
- [Why use SPPS Planner](#why-use-spps-planner)
...
## Why use SPPS Planner

SPPS work often spreads one synthesis across sequence notes, calculator sheets, reagent tables, checklists, cleavage notes, batch summaries, and later analytical results. SPPS Planner keeps those pieces connected around the same peptide/work item instead of treating them as unrelated files.

| Need | SPPS Planner approach |
| --- | --- |
| Turn a peptide design into a practical synthesis plan | Generate an editable stepwise Plan from sequence, resin, scale, loading, and chemistry settings |
| Recalculate after a bench-side change | Edit the visible Plan and use **Apply Change** to update connected calculations |
| Estimate materials before synthesis | Calculate AA/chemical units, reagents, bases, solvents, cleavage components, and batch totals |
| Keep execution history attributable | Link Result / Issue records to the active Work Item and Run context |
| Learn from real experiments without hiding the evidence | Keep measured history, empirical rules, and model output distinguishable |
| Use the public build safely | Start without bundled private laboratory history and build a local evidence base from authorized data |

The program is therefore useful both as a **day-to-day synthesis planner** and as a **local evidence/traceability layer** for repeated peptide work.
...
The final public source package was release-checked with **273 tests passed / 22 skipped**, the release verifier passed, and the Windows release contract passed. The public experimental seed area contains documentation only.
...
- faster Recommendations / Lab History opening while preserving the full evidence/history workflow;
...
The final V5.0.0 public package was validated with **273 passed / 22 skipped**, plus release-contract and Windows-release checks. The repository keeps the regression suite public so behavior changes can be reviewed instead of relying only on a packaged binary.
...
V5.0.0 improves the perceived startup of **Recommendations / Lab History** by avoiding unnecessary repeated database work and by allowing the window to become visible before heavier history views finish refreshing.
...
Window becomes visible promptly
   ↓
Recommendation/history content continues loading as needed
```

The optimization keeps the real recommendation, history, Risk & Evidence, and Data Health workflows intact.
