...
릴리스별 변경사항은 [CHANGELOG.md](CHANGELOG.md)를 참고한다.
...
- Experimental DB migration/backfill 반복 실행 방지
- Recommendations 창을 먼저 띄우고 무거운 데이터는 이후 refresh하는 fast-open 구조
...
V5.0.0은 Recommendations / Lab History 버튼을 눌렀을 때 창이 늦게 뜨는 문제를 직접 다뤘다.

이전 문제의 핵심은 experimental DB initialize 과정에서 canonical lookup key backfill이 반복 실행될 수 있다는 것이었다.

V5 FAST-OPEN 구조에서는:

- experimental DB에 migration/meta version을 저장하고
- 이미 완료된 lookup backfill을 일반 initialize 때 반복하지 않고
- process 내부 initialization fast path를 사용하며
- Recommendations 창은 먼저 생성하고
- 무거운 history/data-health refresh는 창 생성 후 실행한다.
...
창이 먼저 보임
   ↓
필요한 recommendation/history data가 이어서 로드됨
```

가짜 loading placeholder 화면으로 숨기는 것이 아니라 실제 반복 migration 비용을 줄이고 UI population 순서를 개선한 것이다.
...
- [docs/V4_EXPERIMENTAL_ML_KO.md](docs/V4_EXPERIMENTAL_ML_KO.md) — V4에서 V5로 이어진 Experimental Data/ML 설계 배경. 현재 동작 기준은 V5 문서와 코드/테스트다.
...
Experimental Data / ML 구조의 발전 과정을 보여주는 historical reference다. 현재 동작은 V5 코드/테스트/문서를 기준으로 한다.
