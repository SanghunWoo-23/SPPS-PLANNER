...
class ClassicBaseCore(SessionStateMixin, tk.Tk):
    ...
    def _batch_ac_cap_summary(self, rows=None) -> pd.DataFrame:
        ...
        return pd.DataFrame([{'material': 'Acetic anhydride (Ac2O)', 'count': int(count) if float(count).is_integer() else round(count, 3), 'calc_mL': round(calc_ml, 3), 'actual_mL': round(actual_ml, 2), 'MW': mw, 'weight_g': round(actual_g, 3), 'note': ''}], columns=['material', 'count', 'calc_mL', 'actual_mL', 'MW', 'weight_g', 'note'])
    ...
    def _batch_modifier_summary(self, rows=None) -> pd.DataFrame:
        ...
        for r in rows:
            ...
            if not nterm and self._sequence_has_nterm_ac(r.get('Region 1 seq')):
                nterm = 'Ac'
            if nterm:
                add_record('Acetic anhydride (Ac2O)' if nterm == 'Ac' else nterm, 'N-term', copies, scale, note='')
    ...
    def _batch_total_usage_by_category(self, rows=None):
        ...
        for r in rows:
            ...
            if nterm:
                ...
                self._add_total_record(modifier, mat, 'N-term modifier', copies, mod_eq, '', copies * scale * mod_eq, 'mmol', '')
    ...
    def materials_from_rows(self, plan_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, r in plan_df.iterrows():
            ...
            if self._is_ac_unit(name):
                ...
                add(step=step, material='Acetic anhydride (Ac2O)', cls='N-terminal modifier reagent', mmol=mmol, ml=ac_ml, repeat=repeat, phase=phase, note='', src=f'step {step}', mw=ac_mw)
            else:
                mw_unit = meta.get('MW', '')
