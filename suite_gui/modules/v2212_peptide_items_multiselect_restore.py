"""V2.2.12 peptide-item workflow and legacy-control restoration.
...
def delete_items(gui, v229, ns: dict[str, Any]):
    ...
    _save_active(gui, v229, include_outputs=True)
    items = list(getattr(gui, "pm_items", []) or [])
    selected = [i for i in selected if 0 <= i < len(items)]
    if not selected:
        return
    ...
    if not items:
        ...
        new_index = min(selected[0], len(items) - 1)
...
def _move_block(gui, selected: list[int], target: int):
    ...
    if not items or not selected:
        return selected
    selected = sorted(i for i in set(selected) if 0 <= i < len(items))
    if not selected:
        return []
    if target in selected and min(selected) <= target <= max(selected):
        return selected
    active_item = None
    ...
    rest = [item for i, item in enumerate(items) if i not in selected]
    before = sum(1 for i in selected if i < target)
    insert_at = max(0, min(len(rest), target - before))
    gui.pm_items = rest[:insert_at] + block + rest[insert_at:]
    new_selected = list(range(insert_at, insert_at + len(block)))
    if active_item is not None:
        try:
            gui._v229_active_index = gui.pm_items.index(active_item)
        except Exception:
            gui._v229_active_index = new_selected[0]
    _rebuild_listbox(gui, new_selected, _active_index(gui))
    return new_selected
...
def install(gui_cls, ns: dict[str, Any], *_args, **_kwargs):
    ...
    def build(self):
        ...
        self._v2212_ns = ns
        _install_title(self)
        _install_unit_defaults_ui(self)
        _install_checklist_widths(self)
        _install_item_bindings(self, v229, ns)
        try:
            self.pm_list.configure(selectmode=tk.EXTENDED, exportselection=False)
        except Exception:
            pass
        if not getattr(self, "pm_items", None):
            _empty_editor_outputs(self)
        else:
            # Keep one default peptide item active so typing into the blank editor
            # is autosaved to that item, but do not auto-fill the editor on startup.
            if _active_index(self) is None:
                try:
                    self._v229_active_index = 0
                    self.pm_list.selection_clear(0, "end")
                    self.pm_list.selection_set(0)
                    self.pm_list.activate(0)
                except Exception:
                    pass
