...
def _write_rows(tree, rows, columns=None, widths=None):
    ...
    for column in columns:
        ...
        tree.column(column, width=width, minwidth=45, anchor="w", stretch=False)
...
def generate(gui, ns):
    ...
    try:
        ...
        from suite_gui.modules import v227_classic_workflow_restore as legacy_controller
        rows = legacy_controller._stable_plan_rows(gui, inp, ns)
        ...
        legacy_controller._write_checklist_from_operations(gui, tables.get("operations_core", pd.DataFrame()))
        refresh_cleavage(gui, ns, show_errors=False)
...
def _refresh_linked_from_visible_plan(gui, ns):
    ...
    if callable(checklist):
        ...
        try:
            from suite_gui.modules import v227_classic_workflow_restore as legacy_controller
            legacy_controller._write_checklist_from_operations(gui, pd.DataFrame())
        except Exception:
            pass
...
def _install_save_project_button(gui, ns):
    ...
    for widget in global_row.winfo_children():
        if isinstance(widget, ttk.Button) and str(widget.cget("text")) == "Save Project":
            widget.configure(command=lambda: save_project(gui, ns, True))
            return
    ttk.Button(global_row, text="Save Project", command=lambda: save_project(gui, ns, True)).pack(side="left", padx=3)
...
def _install_safe_destroy(gui_cls):
    if getattr(gui_cls, "_v228_destroy_installed", False):
        return
    original = gui_cls.destroy

    def destroy(gui):
        if getattr(gui, "_v228_destroying", False):
            return
        gui._v228_destroying = True
        try:
            for after_id in list(gui.tk.call("after", "info")):
                try:
        ...
        try:
            ...
            gui._v228_destroying = False

    gui_cls.destroy = destroy
    gui_cls._v228_destroy_installed = True


def install(gui_cls, ns, original_build):
    """Install one clean legacy build path and current calculation controllers."""
    def build(gui):
        gui._v228_ns = ns
        gui._v228_switching = True
        original_build(gui)  # one early legacy build; stacked late wrapper builds are bypassed
        gui._v228_switching = False
        _install_editor_traces(gui)
        _hide_non_workbench_tabs(gui)
        _normalize_title(gui)
        _install_result_tabs(gui, ns)
        _install_action_buttons(gui, ns)
        _replace_item_actions(gui, ns)
        _bind_item_buttons(gui)
        # Active index must be established after the legacy builder creates item 0.
        try:
            selected = list(gui.pm_list.curselection())
            gui._v228_active_index = int(selected[0]) if selected else 0
        except Exception:
            gui._v228_active_index = 0
        # Fresh legacy startup stays empty and fast. Generate is the only action
        # that creates/recreates a plan; autosaved per-peptide output is restored
        # only when it actually exists in that peptide item.
        _cancel_pending_legacy_jobs(gui)
        _normalize_setup_panel(gui)
        _make_item_buttons_visible(gui)
        _install_save_project_button(gui, ns)
        if getattr(gui, "pm_items", None):
            item = gui.pm_items[gui._v228_active_index]
            item.update(_editor_payload(gui))
            if item.get("selected_plan_rows"):
                restore_item(gui, gui._v228_active_index, ns)
            else:
                _clear_tree(getattr(gui, "pm_selected_plan_tree", None))
                _clear_tree(getattr(gui, "pm_selected_material_tree", None))
                _clear_tree(getattr(gui, "pm_selected_total_tree", None))
                _clear_tree(getattr(gui, "progress_tree", None))
                _clear_tree(getattr(gui, "pm_cleavage_tree", None))
        # Do not schedule repeated full-tree normalization scans or an automatic
        # calculation during launch.

    gui_cls._build = build
    gui_cls.pm_live_sync_selected = live_sync
    gui_cls.pm_on_select = lambda self, event=None: on_item_select(self, ns, event)
    gui_cls.pm_on_double_click = lambda self, event=None: on_item_select(self, ns, event)
    ...
    gui_cls.save_project = lambda self, show=True: save_project(self, ns, show)
    gui_cls.refresh_outputs_from_tree = _startup_refresh_noop
