...
def test_windows_release_contract_has_no_stale_v2_or_legacy_imports():
    ...
    assert '#define MyAppVersion "6.0.0"' in installer
    assert "VersionInfoVersion=6.0.0.0" in installer
    ...
    assert 'VERSION_NUMBER = "6.0.0"' in version
    assert 'DATA_VERSION = "SPPS data v6.0.0"' in version
